# Adding these first two lines for running plots via ssh
import matplotlib
matplotlib.use('Agg')
...
class HParams():
    def __init__(self):
        # Need to change this to the .npz file we want to train from
        self.data_location = 'The_Eiffel_Tower.full.npz'
...
def max_size(data):
    """largest sequence length in the data set"""
...
def normalize(strokes):
    ...
    for seq in strokes:
        seq[:, 0:2] /= scale_factor # divides every value by the std(data)
        data.append(seq)
    return data
...
class Model():
    ...
    def train(self, epoch):
        ...
        # some print and save:
        if epoch % 1 == 0:
            print('epoch', epoch, 'loss', loss.data, 'LR', LR.data, 'LKL', LKL.data)
